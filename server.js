...
app.get('/api/sessions', async (_req, res) => { const data = await readData(); data.sessions = (data.sessions || []).map(s => { const dismissed = new Set(s.dismissedMistakes || []); const answers = (s.answers || []).filter(a => !dismissed.has(normalizeQuestion(a))); const scored = answers.filter(a => Number.isFinite(Number(a.feedback?.score))); const score = scored.length ? Math.round(scored.reduce((n, a) => n + Number(a.feedback.score), 0) / scored.length) : null; return { ...s, answers, status: scored.length >= (s.questions?.length || 0) && (s.questions?.length || 0) > 0 ? 'completed' : 'in_progress', questionCount: s.questions?.length || 0, answeredCount: scored.length, score, trackName: s.role || '技术岗位' } }); res.json(data) })
...
app.delete('/api/sessions/:id/questions', async (req, res) => { const data = await readData(); const session = data.sessions.find(s => Number(s.id) === Number(req.params.id)); if (!session) return res.status(404).json({ error: '找不到这套训练记录。' }); let rawQuestion = req.body?.question ?? req.body?.questionText ?? req.body?.question_text; let question = normalizeQuestion(rawQuestion); const index = Number.isInteger(Number(req.body?.index)) ? Number(req.body.index) : -1; const before = session.questions?.length || 0; if (index >= 0 && index < before) question = normalizeQuestion(session.questions[index]?.question); if (!session.questions.some(item => normalizeQuestion(item.question) === question)) return res.status(404).json({ error: '找不到要删除的题目。' }); session.dismissedMistakes = [...new Set([...(session.dismissedMistakes || []), question])]; await writeData(data); res.json({ ok: true, sessionId: Number(req.params.id), questionCount: session.questions.length }) })
